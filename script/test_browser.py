...
try:
    ...
    subprocess.run(['open', '-n', str(app), '--stdout', str(profile/'stdout.log'), '--stderr', str(profile/'stderr.log'), '--args', '--lite-test-profile='+str(profile), '--lite-smoke', '--use-mock-keychain'], check=True)
    ...
    if result.get('memory1',{}).get('processes',0) < 3:
        failures.append('incomplete Chromium process-family sample')
    target = root/'test-results'
    target.mkdir(exist_ok=True)
    (target/'engine.json').write_text(json.dumps(result, indent=2)+'\n')
    print(json.dumps(result, indent=2))
    print('FAIL: '+', '.join(failures) if failures else 'PASS: 30 bundled Chromium checks')
